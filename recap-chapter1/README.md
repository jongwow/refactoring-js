...
//TODO: 내용 수정하기. eslint 적용부분 설정이 많이 바뀌어서 다시 적어야할것 같다. 나중에 볼 일 이쓰면 그때 적자.
...
## 1.10 마치며
간단한 예제였지만 이 예제를 통해 **함수 추출하기**, **변수 인라인하기**, **함수 옮기기**, **조건부 로직을 다형성으로 바꾸기**를 사용할 수 있었다. 

 마틴옹: *좋은 코드를 가늠하는 확실한 방법은 '얼마나 수정하기 쉬운가'이다* 

> 퀄리티있는 코드를 작성하는 것은 항상 어려운 길이다. 지금까지 많은 개발 업무가 **유지보수**였기 때문에 특히나 클린 코드, 우아한 코드, 아름다운 코드에 대해 고민이 많았다. 새롭게 시작하는 프로젝트들도 작성하다보면 동작하는 것이 우선시되어 코드 퀄리티를 떨어뜨리는 짓을 많이 했다. 그런 *못된 짓*을 할 때마다 참 아쉽다란 느낌이 들었는데 이번 리팩터링 공부를 통해 못생긴 코드는 작성하지 않는 습관을 들여보고 싶다. 

> 모든 코드는 REFACTORING -> TEST -> COMMIT의 순서로 진행됐습니다. Commit Log를 보면서 따라간다면? *야 너두 할 수 있어! 리팩터링*